...
def get_openai_client() -> Optional[OpenAI]:
    ...
    if not api_key or api_key == "placeholder" or len(api_key) < 20:
        print("⚠️  OpenAI API key not found or invalid. Set OPENAI_API_KEY environment variable.")
        return None
...
async def create_enhanced_pdf(pets: List[Dict], output_dir: Path, temp_dir: Path):
    ...
    for i, pet in enumerate(pets):
        ...
        story.append(Paragraph(f"The Tale of {name}, the Mystical {pet_type.title()} Guardian", title_style))
...
async def main():
    ...
    if not OPENAI_AVAILABLE:
        print("⚠️  OpenAI library not installed. AI images will show placeholders.")
        print("   Install with: pip install openai")
    elif not get_openai_client():
        print("⚠️  OpenAI API key not configured. AI images will show placeholders.")
        print("   Set OPENAI_API_KEY environment variable with your API key.")
    else:
        print("✅ OpenAI API available - AI images will be generated!")
    
    # Create enhanced PDF
    pdf_path = await create_enhanced_pdf(pets, output_dir, temp_dir)
